...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        fan_in = n_features * self.embed_size
        
        self.embed_to_hidden_weight = nn.Parameter(torch.empty(fan_in, hidden_size))
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)

        self.embed_to_hidden_bias = nn.Parameter(torch.empty(1, hidden_size))
        ...
        self.hidden_to_logits_bias = nn.Parameter(torch.empty(1, n_classes))
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.index_select(torch.Tensor(self.embeddings), 0, w.view(-1))
