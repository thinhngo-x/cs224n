...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens, lens = pad_packed_sequence(enc_hiddens, batch_first=True)
        
        last_hidden = last_hidden.permute(1,0,2)
        last_hidden = last_hidden.reshape(last_hidden.shape[0], -1)
        init_decoder_hidden = self.h_projection(last_hidden)
        
        last_cell = last_cell.permute(1,0,2)
        last_cell = last_cell.reshape(last_cell.shape[0], -1)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            ...
            dec_state, o_cur, e_t = self.step(
                Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks
                )
            combined_outputs.append(o_cur)
            o_prev = o_cur
